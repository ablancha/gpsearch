...
from gpsearch.core import BlackBox, GaussianInputs, jacobian_fdiff, \
                          OptimalDesign, Likelihood, IVR_LW, IVR_LWInt
...
def main():
    ...
    myMap = BlackBox(map_def, args=(oscil,), noise_var=0.5)
   ...
   #x_new = X[0:1,:]
    ...
    qcrit = IVR_LW(o.model, o.inputs, likelihood=likelihood)
    print(qcrit.evaluate(x_new))
    print(qcrit.jacobian(x_new))

    qcrit = IVR_LWInt(o.model, o.inputs, ngrid=250, likelihood=likelihood)
    print(qcrit.evaluate(x_new))
    print(qcrit.jacobian(x_new))
